...
namespace Inventory.DataModels.StockTake
{
    public class InventoryUpdateModel
    {
        ...
        {
            InventoryUpdateMaster objMaster = Mediator.GetInstance().GetInventoryUpdate(this.ID);
            ...
            this.Details = Mediator.GetInstance().GetInventoryUpdateDetails(this.ID);
            this.ViewStatus = (objMaster.ViewStatus.IsNull) ? false : (bool)(objMaster.ViewStatus);
        }


        internal InventoryUpdateMaster GetMasterRecord()
        {
